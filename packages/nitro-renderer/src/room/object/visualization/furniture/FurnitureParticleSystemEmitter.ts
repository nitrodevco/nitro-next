import { IGraphicAsset, IVector3D, RoomGeometryScaleType, Vector3d } from '@nitrodevco/nitro-api';
...
export class FurnitureParticleSystemEmitter extends FurnitureParticleSystemParticle {
    ...
    private releaseParticles(particle: FurnitureParticleSystemParticle, direction: IVector3D): void {
        ...
        while (i < this._particlesPerFrame) {
            ...
            if (randomParticle !== undefined) {
                lifeTime = Math.floor(Math.random() * (randomParticle.lifeTime ?? 0) + 10);
                isEmitter = randomParticle.isEmitter ?? false;
                frames = randomParticle.frames ?? [];
                fade = randomParticle.fade ?? false;
            } else {
    ...
    private getRandomParticleConfiguration(): ParticleSystemParticle | undefined {
        return this._particleConfigurations[
            Math.trunc(Math.floor(Math.random() * this._particleConfigurations.length))
        ];
    }
    ...
    public override update(): void {
        ...
        if (!this.isAlive && this._emittedParticles < this._maxNumberOfParticles && this.age % this._burstPulse === 0)
