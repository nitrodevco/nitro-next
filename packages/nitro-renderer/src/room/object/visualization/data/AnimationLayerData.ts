...
export class AnimationLayerData {
    ...
    public getFrame(direction: number, frameCount: number): AnimationFrame | undefined {
        if (this._frameCount < 1) return undefined;
        ...
        if (!this._isRandom) {
            ...
            let sequence: AnimationFrameSequenceData | undefined = undefined;
        ...
        if (!sequence || sequence.frameCount < 1) return undefined;
    ...
    public getFrameFromSequence(
        ...
        if (sequenceId < 0 || sequenceId >= this._frameSequences.length) return undefined;
