...
export class AnimationFrameSequenceData {
    ...
    public addFrame(
        ...
        randomX: number,
        randomY: number,
        directionalOffset: DirectionalOffsetData | undefined,
    ): void {
        let repeats = 1;
    ...
    public getFrame(frameCount: number): AnimationFrameData | undefined {
        if (!this._frames.length || frameCount < 0 || frameCount >= this.frameCount) return undefined;
