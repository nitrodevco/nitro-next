...
import { Matrix, Point, Sprite, Texture, TilingSprite } from 'pixi.js';

import { GetAssetManager } from '../../../../assets';
import { GetRenderer, GetTexturePool } from '../../../../utils';
import { RoomGeometry } from '../../../utils';
import { PlaneMaskFilter, type PlaneMaskManager } from './mask';
...
export class RoomPlane implements IRoomPlane {
    ...
    private _maskManager: PlaneMaskManager | undefined = undefined;
    ...
    private _planeTexture: Texture | undefined = undefined;
    private _maskFilter: Filter | undefined = undefined;
    ...
    public dispose(): void {
        ...
        this._leftSide = undefined!;
        this._rightSide = undefined!;
        this._normal = undefined!;
        this._cornerA = undefined!;
        this._cornerB = undefined!;
        this._cornerC = undefined!;
        this._cornerD = undefined!;
    ...
    public update(geometry: IRoomGeometry, timeSinceStartMs: number, needsUpdate: boolean = false): boolean {
        ...
        if (needsUpdate) {
            ...
            const getTextureAndColorForPlane = (planeId: string, planeType: number) => {
                const dataType: keyof IAssetRoomVisualizationData =
                ...
                const planeVisualization =
                    (dataType === 'landscapeData' ? plane?.animatedVisualization : plane?.visualizations)?.find(
                        visualization => visualization.size === planeGeometry.scale,
                    ) ?? null;
                ...
                const planeAssetName =
                    planeVisualizationData?.textures?.find(texture => texture.id === planeMaterialId)?.bitmaps?.[0]
                        ?.assetName ?? '';
        ...
        if ((needsUpdate || this._maskChanged) && this._planeSprite) {
        ...
        if (this._planeTexture) this._planeTexture.source.label = `room_plane_${this._uniqueId.toString()}`;

        if (needsUpdate && this._planeSprite) {
            GetRenderer().render({
    ...
    private updateMask(container: Container, geometry: IRoomGeometry): boolean {
        ...
        while (i < this._bitmapMasks.length) {
            ...
            if (mask) {
                ...
                this._maskManager.addMaskToContainer(container, mask.type, geometry.scale, normal, posX, posY);
            }
