...
export class RoomRotatingEffect {
    ...
    private static state: number = RoomRotatingEffect.STATE_NOT_INITIALIZED;
    ...
    public static init(delayMs: number, durationMs: number): void {
        ...
        this.state = RoomRotatingEffect.STATE_START_DELAY;
    }

    public static turnVisualizationOn(): void {
        if (this.state === RoomRotatingEffect.STATE_NOT_INITIALIZED || this.state === RoomRotatingEffect.STATE_OVER) {
            return;
        }
        ...
        if (elapsedTime > this.delayDuration + this.duration) {
            this.state = RoomRotatingEffect.STATE_OVER;
            return;
        }
        ...
        this.state = RoomRotatingEffect.STATE_RUNNING;
    ...
    private static isRunning(): boolean {
        return this.state === RoomRotatingEffect.STATE_START_DELAY || this.state === RoomRotatingEffect.STATE_RUNNING;
    }
}