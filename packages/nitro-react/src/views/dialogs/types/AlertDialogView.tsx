...
import { ButtonThick, cn, Frame } from "#base/theme";
...
export const AlertDialogView = ({ dialog, children }: AlertDialogViewProps) => {
    const { id, kind, flags, title, summary } = dialog;
    ...
    const okCaption = t('generic.ok', 'generic.ok');
        ...
        <Frame
            ...
            <div className={ cn('flex items-center mt-4', DIALOG_BUTTON_ALIGNS[kind]) }>
                { hasFlag(DialogFlagEnum.ButtonCancel) && (
                    <DialogLinkButtonView className="text-style-u-regular" onClick={ () => dispatchDialogEvent(id, DialogButtonEnum.Cancel) }>
                        { t('generic.cancel', 'generic.cancel') }
                    </DialogLinkButtonView>
                ) }
                { hasFlag(DialogFlagEnum.ButtonOk) && (kind === DialogKindEnum.AlertLink
                    ? <DialogLinkButtonView className="text-style-u-regular" onClick={ () => dispatchDialogEvent(id, DialogButtonEnum.Ok) }>{ okCaption }</DialogLinkButtonView>
