 ...
 */
export const useRoomFriendFurniHandler = () => {
    const request = useRoomWidget<LockConfirmData>(RoomObjectWidgetRequestEvent.FRIEND_FURNITURE_CONFIRM);
    const { openRoomWidget, updateRoomWidgetData, mergeRoomWidgetData, closeRoomWidget } = useRoomWidgetActions();

    useMessageListener(FriendFurniStartConfirmationMessage, (data) => {
        openRoomWidget({
    ...
    useMessageListener(FriendFurniOtherLockConfirmedMessage, (data) => {
        if (request?.objectId !== data.stuffId) return;

        mergeRoomWidgetData<LockConfirmData>(RoomObjectWidgetRequestEvent.FRIEND_FURNITURE_CONFIRM, { otherLocked: true });
    });

    useMessageListener(FriendFurniCancelLockMessage, (data) => {
        if (request?.objectId !== data.stuffId) return;
