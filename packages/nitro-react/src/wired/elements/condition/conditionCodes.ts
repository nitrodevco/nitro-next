/**
 * `conditions/ConditionCodes` mirrored whole - the `code` the server sends in a
 * `ConditionDefinition`, values from the `.as` file. A condition element serves its `code` and,
 * where it has one, the `NOT_` twin as its `negativeCode` (the server inverts the result).
 * 9 and 20 (the server's `STUFFS_IN_FORMATION` pair) are not in the Flash class. One member is
 * obfuscated in the 2026 dump; its name comes from the element that returns it.
 */
export const ConditionCodes = {
    STATES_MATCH: 0,
    FURNIS_HAVE_AVATARS: 1,
    TRIGGERER_IS_ON_FURNI: 2,
    TIME_ELAPSED_MORE: 3,
    TIME_ELAPSED_LESS: 4,
    USER_COUNT_IN: 5,
    ACTOR_IS_IN_TEAM: 6,
    HAS_STACKED_FURNIS: 7,
    STUFF_TYPE_MATCHES: 8,
    ACTOR_IS_GROUP_MEMBER: 10,
    ACTOR_IS_WEARING_BADGE: 11,
    ACTOR_IS_WEARING_EFFECT: 12,
    NOT_STATES_MATCH: 13,
    NOT_FURNIS_HAVE_AVATARS: 14,
    NOT_TRIGGERER_IS_ON_FURNI: 15,
    NOT_USER_COUNT_IN: 16,
    NOT_ACTOR_IS_IN_TEAM: 17,
    NOT_HAS_STACKED_FURNIS: 18,
    NOT_STUFF_TYPE_MATCHES: 19,
    NOT_ACTOR_IS_GROUP_MEMBER: 21,
    NOT_ACTOR_IS_WEARING_BADGE: 22,
    NOT_ACTOR_IS_WEARING_EFFECT: 23,
    DATE_RANGE_ACTIVE: 24,
    ACTOR_HAS_HANDITEM: 25,
    TRIGGERER_MATCHES: 26,
    NOT_TRIGGERER_MATCHES: 27,
    TIME_MATCHES: 28,
    DATE_MATCHES: 29,
    NOT_HAS_HANDITEM: 30,
    TEAM_IS_WINNING: 31,
    PERFORMING_ACTION: 32,
    NOT_PERFORMING_ACTION: 33,
    TEAM_HAS_SCORE: 34,
    CLOCK_TIME_MATCHES: 35,
    FURNI_HAS_ALTITUDE: 36,
    USER_DIRECTION: 37,
    INPUT_SOURCE_QUANTITY: 38,
    CAN_PERFORM_MOVE: 39,
    /** `§_-M2N§` - `§_-X1P§` (`HasVariable`), the positive twin of `NOT_HAS_VARIABLE`. */
    HAS_VARIABLE: 40,
    NOT_HAS_VARIABLE: 41,
    VARIABLE_VALUE: 42,
    VARIABLE_AGE: 43,
    USER_LEVEL: 44,
    CHEST_HAS_ITEMS: 45,
    CHEST_HAS_ITEM_TYPES: 46,
} as const;
